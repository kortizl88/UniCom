...
AS
  ...
  *************************************************************/
  ...
  FUNCTION FNCONSULTAINFOEMPLEADO(
      ...
      paTienda       IN UNIFORMES.TATIENDAS.FISUCURSAL%TYPE)
  RETURN rcgCursor;
...
CREATE OR REPLACE PACKAGE BODY                      UNIFORMES.PAWEBUNIFORMESCOMERCIO
   ...
   *************************************************************/
    ...
    csgDescSap                  CONSTANT NUMBER(1):=8;  -- Descuento SAP
    csgDescSppi                 CONSTANT NUMBER(1):=9;  -- Descuento SPPI
    ...
    csgCadenaDescSap            CONSTANT VARCHAR2(50 CHAR):='DESCUENTO (SAP)';      -- Descuento SAP
    csgCadenaDescSppi           CONSTANT VARCHAR2(50 CHAR):='DESCUENTO (SPPI)';     -- Descuento SPPI
  ...
  *************************************************************/
  ...
  vlFnGen NUMBER(4):= 8030;
  vlNegGen NUMBER(1):= 1;
  vlUsAct VARCHAR2(1 CHAR) := 'A';
  vlPrefComercio VARCHAR2(2 CHAR):= '52';
  vlTiendaCC VARCHAR2(10 CHAR);
  vlStrTienda VARCHAR2(5 CHAR):= TO_CHAR(paTienda);
  vlPadCero VARCHAR2(1 CHAR):= '0';
  
  BEGIN
  	
  	-- Obtiene tienda para generalista
  	vlTiendaCC := vlPrefComercio || ( CASE WHEN LENGTH(vlStrTienda) = csgTres THEN vlPadCero||vlStrTienda ELSE vlStrTienda END);
    ...
    OPEN curCursorSalida FOR
      SELECT E.FIEMPLEADO
            ...
            ,FCSITUACION
            ,FIFUNCION
            ,CASE WHEN F.FIIDNEGOCIO IS NULL AND E.FIFUNCION = vlFnGen THEN vlNegGen ELSE F.FIIDNEGOCIO END FIIDNEGOCIO
            ,F.FISOLICITUDPLANTILLA
            ,N.FISOLICITAENTIENDA
            ,T.FIPAIS
            ,T.FICANAL
            ,T.FISUCURSAL FICECO
            ,CASE WHEN E.FIFUNCION = vlFnGen THEN TO_NUMBER(vlTiendaCC) ELSE E.FICECO END CENTROCOSTOS
            ,T.FCNOMBRE FCCECO
            ,FCPAIS
            ,TO_CHAR(FDINGRESO,'DD/MM/YYYY') FDINGRESO
        FROM UNIFORMES.TAEMPLEADOS E
   LEFT JOIN UNIFORMES.TATIENDAS T
          ON T.FISUCURSAL = paTienda
 ...
 *************************************************************************************************************/   
	...
	INNER JOIN UNIFORMES.TACARGASSEMESTRALES C 
	 ...
	 INNER JOIN UNIFORMES.TAMENUS M 
	      ...
	      WHERE MFXN.FIFUNCIONSAP = paNoFuncionSap
	        ...
	        AND MFXN.FIIDNEGOCIO = paNoNegocio;
  ...
  *************************************************************/
  ...
  INNER JOIN UNIFORMES.TATIPOKIT K
          ON F.FIKIT = K.FIKIT
    ...
    LOOP
    	...
    	END;
    		...
    		IF paSolicitudes(indSol).FIIDTIPOSOLICITUD = vlSolicitudSem THEN
			 ...
			 INNER JOIN UNIFORMES.TACARGASXNEGOCIO N
			         ON C.FIIDCARGA = N.FIIDCARGA
			  	  WHERE SYSDATE BETWEEN C.FDFECHAINICIAL AND C.FDFECHAFINAL
			  	    AND C.FIESTATUS = csgUno
			  	    AND N.FIIDNEGOCIO = paSolicitudes(indSol).FIIDNEGOCIO;
 ...
 BEGIN  
 OPEN curDatos FOR        
     SELECT TSD.FIPEDIDO FINUMPEDIDO,
 ...
 BEGIN
    vlCadenaDescripcionEstatus :=   (CASE paNoEstadoNuevo
                                        ...
                                        WHEN csgDescSap           THEN csgCadenaDescSap
                                        WHEN csgDescSppi          THEN csgCadenaDescSppi
                                     END);
 ...
 *************************************************************/
 ...
 OPEN curDatos FOR
     SELECT TPCD.FIPEDIDO,
            ...
            TP.FCDESCRIPCION,
            TO_CHAR((FNTOTALDESCUENTO - FNSALDO),'FM$99990.00') AS FCPAGADO,
            TPCD.FICANTIDAD,
            TO_CHAR(TDS.FNTOTALDESCUENTO,'FM$99990.00') AS FCCOSTOPRENDA,
            TDS.FISEMANADESCUENTO AS FISEMANA,
       ...
       FROM UNIFORMES.TADESCUENTOSSAPXPEDIDO TDS
 INNER JOIN (SELECT FIFOLIOSOLICITUD,
                    FIPEDIDO, 
                    MAX(FDPAGO) AS FDPAGO 
               FROM UNIFORMES.TADESCUENTOSSAPXPEDIDO 
              WHERE FIFOLIOSOLICITUD = paFolioSolicitud 
                AND ( CASE 
                      ...
                      CASE 
                        WHEN ((FISEMANADESCUENTO) <= (SELECT (TO_NUMBER(TO_CHAR(SYSDATE, 'WW'))) FROM DUAL)) THEN csgUno
                        ELSE csgCero
                      END
                        ELSE csgCero
                      END ) = csgUno
                 GROUP BY FIFOLIOSOLICITUD,FIPEDIDO) TDSGROUPBY
           ON TDS.FIFOLIOSOLICITUD = TDSGROUPBY.FIFOLIOSOLICITUD 
          AND TDS.FIPEDIDO = TDSGROUPBY.FIPEDIDO 
          AND TDS.FDPAGO = TDSGROUPBY.FDPAGO
   INNER JOIN UNIFORMES.TASOLICITUDESDETALLE TPCD
           ON TDSGROUPBY.FIFOLIOSOLICITUD = TPCD.FIFOLIOSOLICITUD 
 ...
 OPEN curDatos FOR
    ...
    SELECT TO_CHAR(TDSAP.FDPAGO, 'YYYY') AS FCANO,
           TDSAP.FISEMANADESCUENTO,
           TO_CHAR(TDSAP.FDPAGO) AS FCFECHAPAGO,
           TO_CHAR(TDSAP.FNTOTALDESCUENTO,'FM$99990.00') AS FCCOSTOPRENDA,
...
INNER JOIN UNIFORMES.TADESCUENTOSSAPXPEDIDO TDSAP
        ON TDSAP.FIFOLIOSOLICITUD = TPCD.FIFOLIOSOLICITUD
 ...
 *************************************************************************************************************/   
    ...
    FOR indice IN 1..paConfirmacionesEntrega.count
        LOOP    
           ...
           UNIFORMES.PAWEBUNIFORMESCOMERCIO.SPGUARDABITACORA(paConfirmacionesEntrega(indice).FIFOLIOSOLICITUD,
           													 ...
           													 paConfirmacionesEntrega(indice).FCDATOS, 
 ...
 /************************************************************************************************************
    Proyecto:              Uniformes Comercio
    Descripcion:           Cancela solicitud
    Parametros de entrada: folio solicitud
    Parametros de salida:  Respuesta del Stored Procedure
    Parametros de ent-sal: No aplica
    Valor de retorno:      No aplica 
 ...
 *************************************************************************************************************/ 
 ...
 BEGIN
	 ...
	 UPDATE UNIFORMES.TASOLICITUDES
