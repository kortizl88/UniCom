...
AS
  ...
  *************************************************************/
  ...
  BEGIN
    ...
    OPEN curCursorSalida FOR
      SELECT S.FIIDEMPLEADO,
      		 E.FCNOMBRE,
		 TS.FCDESCRIPCION TIPOSOLICITUD,
      		 S.FIFOLIOSOLICITUD,
      		 D.FIPAIS,
      		 D.FICANAL,
      		 D.FISUCURSAL,
  ...
  INNER JOIN UNIFORMES.TAEMPLEADOS E
  	  ON S.FIIDEMPLEADO = E.FIEMPLEADO
  INNER JOIN UNIFORMES.TATIPOSOLICITUD TS
          ON S.FIIDTIPOSOLICITUD = TS.FIIDTIPOSOLICITUD
  ...
  *************************************************************/
  ...
  BEGIN
    OPEN curCursorSalida FOR
      SELECT 	B.FIFOLIOSOLICITUD,
				...
				D.FISUCURSAL,
				D.FIPEDIDO,
				CASE WHEN paDatosProceso = csgUno THEN TO_CHAR(DBMS_LOB.SUBSTR(B.FCDATOSPROCESO,vlLongDatosProc,csgUno)) ELSE ' ' END FCDATOSPROCESO,
				B.FCCOMENTARIOS,
				EA.FCDESCRIPCION EST_ANT,
				EN.FCDESCRIPCION EST_NUEVO,
				TO_CHAR(B.FDFECHA, 'DD/MM/YYYY HH24:MI:SS') FDFECHA,
				(CASE WHEN NCE.FINOTAENTRADA IS NULL THEN 0 ELSE FIFOLIOREMISION END) AS FIFOLIOREMISION,
                (CASE WHEN NCE.FINOTACARGO IS NULL THEN 0 ELSE FIFOLIOREMISION END) AS FINOTACARGO,
                (CASE WHEN NCE.FCOBSERVACIONNC IS NULL THEN ' ' ELSE FCOBSERVACIONNC END)  AS FCOBSERVACIONNC,
                (CASE WHEN NCE.FCOBSERVACIONNE IS NULL THEN ' ' ELSE FCOBSERVACIONNE END)  AS FCOBSERVACIONNE
  ...
  INNER JOIN UNIFORMES.TAESTATUSPEDIDOS EN
   ...
   LEFT JOIN UNIFORMES.TANOTASCARGOENTRADA NCE
          ON NCE.FIFOLIOSOLICITUD = D.FIFOLIOSOLICITUD AND
             NCE.FIIDDETALLE = D.FIIDDETALLE AND
             NCE.FIPEDIDO = D.FIPEDIDO AND
             NCE.FIPAIS = D.FIPAIS AND
             NCE.FICANAL = D.FICANAL AND
             NCE.FISUCURSAL = D.FISUCURSAL AND
             NCE.FISKU = D.FISKU
