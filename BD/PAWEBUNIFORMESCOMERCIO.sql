 ...
 *************************************************************************************************************/   
  ...
  *************************************************************/
  ...
  BEGIN
  		/*OBTIENE ULTIMA SOLICITUD VOLUNTARIA*/
    	SELECT NVL(MAX(FDFECHACAPTURA), ( SYSDATE - ( vlDiasParaSolicitar + 1 )) )
    	  INTO vlUltimaSolicitud
    	  FROM UNIFORMES.TASOLICITUDES
    	 WHERE FIIDEMPLEADO = paNumEmp
    	   AND FIIDTIPOSOLICITUD = csgUno;
    
    IF vlUltimaSolicitud > SYSDATE - vlDiasParaSolicitar AND paTipoSol = 1 THEN 
    	OPEN curCursorSalida FOR
    	SELECT 'Aùn no han pasado 8 semanas desde tu ùltima solicitud semestral' MSG, 1 ERROR
    	  FROM DUAL;
    	RETURN curCursorSalida;
    END IF;
  ...
  *************************************************************/
  ...
  BEGIN
	...
	INNER JOIN UNIFORMES.TAREMISIONESXPEDIDO RM
		   ...
		   AND RM.FISUCURSAL = P.FISUCURSAL
...
--   USO RANK OVER    
    ...
    SELECT  TSD.FIPEDIDO,
     ...
     WHERE  TSD.FIACTIVO = csgUno AND TSD.FIFOLIOSOLICITUD = paNoFolioSolicitud;        
 RETURN curDatos;
 ...
 */
    SELECT  FNCADENAPORCENTAJE(CANCELADO * vlCalculoPorcentaje)         AS FCPORCCANCELADO,
            FNCADENAPORCENTAJE(SOLICITUDGENERADA * vlCalculoPorcentaje) AS FCPORCSOLICITUDGENERADA,
            FNCADENAPORCENTAJE(SOLICITADOTIENDA * vlCalculoPorcentaje)  AS FCPORCSOLICITADOTIENDA,
            FNCADENAPORCENTAJE(SOLICITADOCD * vlCalculoPorcentaje)      AS FCPORCSOLICITADOCD,
            FNCADENAPORCENTAJE(ATENDIDOCD * vlCalculoPorcentaje)        AS FCPORCATENDIDOCD,
