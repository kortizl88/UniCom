...
public class DAOTracking {
    ...
    public ArrayList<Tracking> getTrackings(Integer folioSolicitud) throws Exception {                           
        ...
        ArrayList<Tracking> trackings;
                System.out.println("ID getTrackings "+ folioSolicitud);
        try {
            conn = fabricaDAO.getConexion();
            if (conn == null) {
                throw new Exception("La conexion no se creo.");
            }           
            cs = conn.prepareCall(funcionesBD.FN_CONS_SOLICITUD);            
            ...
            trackings = (ArrayList<Tracking>) m.mapperArrayBean(rs, Tracking.class);            
            ...
            throw new Exception("ERROR en : " + this.getClass() + " metodo: getTrackings " + e.getMessage());
        } finally {
            close(conn, cs, rs);
        }        
        return trackings;
